	...
	render() {
		...
		return(
			<div className='searShop'>
				<ul>
					{
						props.shopItem.map((item, index) => {
							return(
								<Link to={'/detail/' + item.goods.gid}  key={index}>
									<li>
									<div className='shop'>
										<img src={item.goods.image} className='shop' alt=''/>
									</div>
									...
									<div className="right">
										...
										<p className="p3">
											...
											<span className="num">{item.goodsStock.saled}人购买</span>
										</p>
										<p className="p4">
											<span className="now" id='shop_price'>￥{item.goods.vipshopPrice}</span>
											<span className="old">
...
const Shop = connect(mapStateToProps, mapDispatchToProps)(ShopUI);
export default Shop;